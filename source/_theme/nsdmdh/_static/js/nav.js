...
// Hide local toc entries in sidebar
const internalLinks = document.querySelector('.sphinxsidebar').querySelectorAll('a[href^="#"]')
for (i in internalLinks){
  if (i == 0){ 
    // do nothing
  } else {
    try {
    internalLinks[i].parentNode.classList.add('hidden')
    } catch(err) {
      // do nothing
      // console.log('caught')
    }
